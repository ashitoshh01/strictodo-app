...
-- Create demo guest account in auth.users only if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'guest@doordue.com') THEN
...
-- Create profile for the guest demo user with 10,000 due coins only if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.profiles WHERE id = '22222222-2222-2222-2222-222222222222'::uuid) THEN
  ...
  ELSE
    UPDATE public.profiles 
    SET due_coins = 10000, welcome_bonus_claimed = true
